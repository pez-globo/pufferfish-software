...
def initialize_states(all_states: MutableMapping[
    ...
    for segment_type in all_states:
        if segment_type is backend.StateSegment.SENSOR_MEASUREMENTS:
        ...
        elif segment_type is backend.StateSegment.PARAMETERS_REQUEST:
        ...
        elif segment_type is backend.StateSegment.ALARM_MUTE:
            all_states[segment_type] = mcu_pb.AlarmMute(
                active=False, remaining=120
            )
        elif segment_type is backend.StateSegment.ALARM_MUTE_REQUEST:
            all_states[segment_type] = mcu_pb.AlarmMute(
                active=False, remaining=120
            )
        elif segment_type is backend.StateSegment.BATTERY_POWER:
            all_states[segment_type] = mcu_pb.BatteryPower(
                power_left=0, charging_status=False
            )
        elif segment_type is backend.StateSegment.SCREEN_STATUS:
            all_states[segment_type] = mcu_pb.ScreenStatus(lock=False)
        elif segment_type is backend.StateSegment.SYSTEM_SETTING_REQUEST:
            all_states[segment_type] = frontend_pb.SystemSettingRequest(
                brightness=100, date=int(time.time())
            )
        elif segment_type is backend.StateSegment.FRONTEND_DISPLAY:
            all_states[segment_type] = frontend_pb.FrontendDisplaySetting(
