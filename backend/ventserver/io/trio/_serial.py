...
class Driver(endpoints.IOEndpoint[bytes, bytes]):
    ...
    _stream: Optional[hazmat.FdStream] = attr.ib(default=None, repr=False)
    ...
    async def open(self, nursery: Optional[trio.Nursery] = None) -> None:
        ...
        try:
            ...
            self._stream = hazmat.FdStream(os.dup(self._connection.fileno()))
    ...
    async def receive(self) -> bytes:
        ...
        try:
            while True:
                ...
                if read:
                    break
                await hazmat.wait_readable(self._stream.fileno())
        except (trio.BrokenResourceError, trio.ClosedResourceError):
            await self.close()
            raise BrokenPipeError(
