...
SPIDeviceStatus HALSPIDevice::read(uint8_t *buf, size_t count) {
  HAL_StatusTypeDef stat =
      HAL_SPI_Receive(&dev_, buf, count, HALSPIDevice::default_timeout);
  if (stat == HAL_OK) {
...
SPIDeviceStatus HALSPIDevice::write(uint8_t *buf, size_t count) {
  HAL_StatusTypeDef stat =
      HAL_SPI_Transmit(&dev_, buf, count, HALSPIDevice::default_timeout);
  if (stat == HAL_OK) {
    return SPIDeviceStatus::ok;
  }
...
SPIDeviceStatus HALSPIDevice::write_read(uint8_t *tx_buf, uint8_t *rx_buf,
                                         size_t count) {
  HAL_StatusTypeDef stat = HAL_SPI_TransmitReceive(
      &dev_, tx_buf, rx_buf, count, HALSPIDevice::default_timeout);
  if (stat == HAL_OK) {
    return SPIDeviceStatus::ok;
  }
  if (stat == HAL_BUSY) {
...
void HALSPIDevice::chip_select(bool input) { cs_pin_.write(input); }
