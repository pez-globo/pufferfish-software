...
void AlarmsService::transform(
  ...
  switch (edge) {
    case EdgeState::rising_edge:
      alarms_manager.deactivate_alarm(Application::LogEventCode_mcu_backend_connection_down);
      log_manager.add_event(
          Application::LogEventCode_mcu_backend_connection_up, LogEventType::LogEventType_system);
      break;
    case EdgeState::falling_edge:
      alarms_manager.activate_alarm(
          Application::LogEventCode_mcu_backend_connection_down, LogEventType::LogEventType_system);
      break;
    default:
      return;
  }
