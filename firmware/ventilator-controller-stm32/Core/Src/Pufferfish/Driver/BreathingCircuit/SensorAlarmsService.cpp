...
void SensorAlarmsService::transform(
  ...
  if (!sensor_connections.sfm3019_air_connected) {
    alarms_manager.activate_alarm(
        Application::LogEventCode_sfm3019_air_disconnected, LogEventType::LogEventType_system);
  } else {
    alarms_manager.deactivate_alarm(Application::LogEventCode_sfm3019_air_disconnected);
  }
  if (!sensor_connections.sfm3019_o2_connected) {
    alarms_manager.activate_alarm(
        Application::LogEventCode_sfm3019_o2_disconnected, LogEventType::LogEventType_system);
  } else {
    alarms_manager.deactivate_alarm(Application::LogEventCode_sfm3019_o2_disconnected);
  }
}
