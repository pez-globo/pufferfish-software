...
void AlarmsService::transform(
  ...
  if (!mcu_power_status.charging) {
    alarms_manager.activate_alarm(
        Application::LogEventCode_charger_disconnected, LogEventType::LogEventType_system);
  } else {
    alarms_manager.deactivate_alarm(Application::LogEventCode_charger_disconnected);
  }
  ...
  switch (battery_level_) {
    case BatteryLevel::critical:
      alarms_manager.activate_alarm(
          Application::LogEventCode_battery_critical, LogEventType::LogEventType_system);
      alarms_manager.deactivate_alarm(Application::LogEventCode_battery_low);
      break;
    case BatteryLevel::low:
      alarms_manager.activate_alarm(
          Application::LogEventCode_battery_low, LogEventType::LogEventType_system);
      alarms_manager.deactivate_alarm(Application::LogEventCode_battery_critical);
      break;
