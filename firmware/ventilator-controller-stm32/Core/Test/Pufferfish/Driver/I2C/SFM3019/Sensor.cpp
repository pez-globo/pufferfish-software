...
//     mock_device.add_read(read_buffer1.data(), read_buffer.size(), write_status);
//     auto read_buffer3 = PF::Util::Containers::make_array<uint8_t>(0x01, 0x02);
//     global_device.add_read(read_buffer3.data(), read_buffer3.size(), write_status);
//     WHEN("The setup method is called once ,time is set to 100us") {
//       PF::Driver::I2C::SFM3019::Sensor sensor{device, false, time};
...
// // SCENARIO(
// //     "The Sensor::setup method works correctly when mock read buffer is different than
// expected, "
// //     "setup method retries for maximum allowed times") {
// //   // wrong data for product id, after 8 fault_counts it fails
// //   GIVEN("SFMC3019 sensor is created with mock I2C device with read buffer consisting of[00 aa
// a6
// //   a0"
// //       " 00 7e 01 48 f1]  (as  invalid read_product_id response) reseter is set to false and"
// //       "mock time set to 100us, global device's mock write buffer is " "set to [01] byte and
// read
...
// //       size_t count = buffer_size;
// //       mock_device.get_write(input_buffer.buffer(), count);
// //       THEN("The mock_device I2C's write buffer contains request_product_id command byte [E1
// 02]")
// //       {
// //         REQUIRE(input_buffer[0] == 0xE1);
// //         REQUIRE(input_buffer[1] == 0x02);
...
// on
// //     next count
// //   GIVEN("SFMC3019 sensor is created with mock I2C device with read buffer consists of 4
// //   repetition"
// //       " of[00 aa a6 a0 00 " "7e 00 00 81](as invalid read_product_id response) [04 02 60 06
// 11"
// //       "a9 00 00 81] (as valid " "read_product_id " "response) " "and[00 aa a6 a0 00 7e 00 00"
// //       "81](as valid read_conversion_factor response) [15 35 " "a8](as valid read_sample
// //       response)" "followed by junk bytes [01 02], reseter is set to " "false and mock time set
// to
// //       100us," "global device's mock write buffer is set to [01] byte and " "read buffer is set
// to
// //       [01 02] bytes") {
// //     PF::HAL::Mock::I2CDevice mock_device;
// //     PF::HAL::Mock::I2CDevice global_device;
...
// //         REQUIRE(status3 == PF::InitializableState::ok);
// //       }
// //       THEN("After the third setup method call, get_state method returns wait_measurement
// action")
// //       {
// //         auto state1 = sensor.get_state();
// //         REQUIRE(state1 == PF::Driver::I2C::SFM3019::StateMachine::Action::wait_measurement);
...
// //   // wrong read buffer for read_product_id conditions(first 7 fault_counts fails), it passes
// on
// //   last count GIVEN("SFMC3019 sensor is created with mock I2C device with read buffer consists
// of
// //   7 repetition"
// //       "of[00 aa a6 a0 00 7e 00 00 81] bytes (as invalid response)  [04 02 60 06 11 a9 00 00
// 81]
...
// //      WHEN("The setup method is called thrice , where  mock time is set to [100, 55000,
// 55200]us
// //      before each corresponding setup call") {
// //          PF::Driver::I2C::SFM3019::Sensor sensor{device, false, time};
// //          time.set_micros(100);
...
// //              REQUIRE(status == PF::InitializableState::setup);
// //             }
// //          THEN("After the first setup method call, get_state method returns wait_warmup
// action") {
// //              auto state = sensor.get_state();
// //              REQUIRE(state == PF::Driver::I2C::SFM3019::StateMachine::Action::wait_warmup);
// //             }
...
// //              REQUIRE(status1 == PF::InitializableState::setup);
// //             }
// //          THEN("After the second setup method call, get_state method returns check_range
// action")
// //          {
// //              auto state2 = sensor.get_state();
// //              REQUIRE(state2 == PF::Driver::I2C::SFM3019::StateMachine::Action::check_range);
...
//       " "set to 100us, global device's mock write buffer is set to [01] byte and read" "buffer is
//       set to [01 02] bytes") {
//     PF::HAL::Mock::I2CDevice mock_device;
//     PF::HAL::Mock::I2CDevice global_device;
//     auto write_status = PF::I2CDeviceStatus::ok;
...
// //   GIVEN("SFMC3019 sensor is partially initialised by calling setup method once where it
// returns "
// //       "setup state and get_state returns wait_warmup, with the mock read buffer consisting of
// [d5
// //       64 4d](as " "read_sample response) and with the mock time set to 500us, global device's
// //       mock write " "buffer is set to [01] byte and read buffer is set to [01 02] bytes" "and
...
// //      REQUIRE(input2[0] == 0x36);
// //      REQUIRE(input2[1] == 0x03);
// //      WHEN("The output method is called once, mock time is set to 250us and output parameter
// flow
// //      is initialized to 20.5") {
// //          time.set_micros(250);
// //          auto output_status = sensor.output(flow);
...
// //         }
// //     }
// //   GIVEN("SFMC3019 sensor is partially initialised by calling setup method multiple times until
// it
// //   "
// //       "returns setup and get_state returns check_range with the mock read buffer consisting of
// //       [d5 64 4d](as " "read_sample response) and with the mock time set to 500us, global
// device's
// //       mock write " "buffer is set to [01] byte and read buffer is set to [01 02] bytes" "and
// //       mock_device I2C's write buffer contains command byte [E1 02](request_product_id), [36
// //       61](read_conversion), [36 6A](set_averaging) and [36 03](O2 gas)") {
