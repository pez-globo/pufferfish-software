...
SCENARIO("The StateMachine::update method works correctly when update is called multiple times") {
  GIVEN(" A freshly constructed state machine") {
    ...
    WHEN("The update method is called thrice with [4000, 100, 5000]us as input parameter") {
      // Action returns wait_measurement for normal input time
      uint32_t time = 4000;
      auto status1 = state_machine.update(time);
...
SCENARIO(
  ...
  GIVEN("The freshly constructed state machine") {
    PF::Driver::I2C::SFM3019::StateMachine state_machine;
    WHEN("The update method is called two times with [0, 29999]us as input parameter ") {
      // for edge cases, action remains wait_warmup
      uint32_t time = 0;
      auto status1 = state_machine.update(time);
      THEN("The first update method call returns wait_warmup action") {
        REQUIRE(status1 == PF::Driver::I2C::SFM3019::StateMachine::Action::wait_warmup);
    ...
    WHEN(
        "The update method is called four times with [0, 30001, 30002, 31000]us as input "
        "parameter") {
...
SCENARIO(
  ...
  GIVEN("The freshly constructed state machine") {
    ...
    WHEN(
        "The update method is called four times with [500, 250, 300]us as input "
...
SCENARIO(
  ...
  GIVEN(" A freshly contructed state machine") {
    ...
    WHEN(
        "The update method is called four times with [100, 299, 300, 301]us as input "
        "parameter ") {
...
SCENARIO(
    "The Sensor::setup method works correctly when enough time is elapsed between setup calls") {
  GIVEN(
...
SCENARIO("The Sensor::setup method works correctly when enough time is not elapsed") {
  GIVEN(
    ...
    // action still remains  wait_measurement
    WHEN(
      ...
      auto status4 = sensor.setup();
      THEN("The third setup method call retuns ok status") {
        REQUIRE(status4 == PF::InitializableState::ok);
      }
      THEN("After the third setup method call, get_state method returns wait_measurement action") {
        auto state1 = sensor.get_state();
        REQUIRE(state1 == PF::Driver::I2C::SFM3019::StateMachine::Action::wait_measurement);
...
SCENARIO(
  ...
  GIVEN(
      ...
      "7e 00 00 81](invalid response) [00 aa a6 a0 00 7e 00 00 81](invalid response) [00 aa a6 a0 "
      "00 7e 00 00 81](invalid response) [00 aa a6 a0 00 7e 00 00 "
      "81](invalid response) [04 02 60 06 11 "
  ...
  GIVEN(
      ...
      "7e 00 00 81](invalid response) [00 aa a6 a0 00 7e 00 00 81](invalid response) [00 aa a6 a0 "
      "00 7e 00 00 81](invalid response) [00 aa a6 a0 00 7e 00 00 "
      "81](invalid response)[00 aa a6 a0 00 7e 00 00 81](invalid response)[00 aa a6 a0 00 7e 00 00 "
      "81](invalid response)[00 aa a6 a0 00 7e 00 00 81](invalid response) [04 02 60 06 11 "
      "a9 00 00 81] (as a valid read_product_id response) "
      "and[00 aa a6 a0 00 7e 00 00 81](as read_conversion_factor response) [15 35 "
      "a8](as read_sample response) followd by junk bytes [01 02], reseter is set to "
      "false and mock time set to 100us, global device's mock write buffer is set to [01] byte") {
  ...
  GIVEN(
      "SFMC3019 sensor is created with mock I2C device with read buffer consists of[04 02 60 06 11 "
      "a9 00 00 81] (as a valid read_product_id response) "
      "and[04 02 60 06 11 a9 00 00 81](as invalid read_conversion_factor response) [04 02 60 06 11 "
      "a9 00 00 81](as invalid read_conversion_factor response), reseter is set to "
      "[04 02 60 06 11 a9 00 00 81](as invalid read_conversion_factor response),[04 02 60 06 11 a9 "
      "00 00 81](as invalid read_conversion_factor response), [04 02 60 06 11 a9 00 00 81](as "
      "invalid read_conversion_factor response),"
      "[04 02 60 06 11 a9 00 00 81](as invalid read_conversion_factor response) and[00 aa a6 a0 00 "
      "7e 00 00 81](as valid read_conversion_factor response) [15 35 "
      "a8](as read_sample response) followd by junk bytes [01 02], reseter is set to "
      "false and mock time set to 100us, global device's mock write buffer is set to [01] byte") {
    PF::HAL::Mock::I2CDevice mock_device;
    PF::HAL::Mock::I2CDevice global_device;
