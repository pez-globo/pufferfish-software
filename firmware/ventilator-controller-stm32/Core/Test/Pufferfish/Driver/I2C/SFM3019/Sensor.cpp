...
SCENARIO("The Sensor::setup method works correctly when resetter is set to true") {
  GIVEN(
      ...
      "a8](as valid read_sample response) followed by junk bytes [01 02], reseter is set to "
      "true and mock time is set to 100us,"
...
//   GIVEN("SFMC3019 sensor is created with mock I2C device with read buffer consisting of[00 aa a6
//   a0"
...
//       response)" "followed by junk bytes [01 02], reseter is set to " "false and mock time set to
...
//   last count GIVEN("SFMC3019 sensor is created with mock I2C device with read buffer consists of
//   7 repetition"
...
//      WHEN("The setup method is called thrice , where  mock time is set to [100, 55000, 55200]us
//      before each corresponding setup call") {
//          PF::Driver::I2C::SFM3019::Sensor sensor{device, false, time};
//          time.set_micros(100);
//          auto status = sensor.setup();
...
SCENARIO(
  ...
  GIVEN(
      ...
      "returns wait_measurement with the mock read buffer consisting of [97 38 1e 97 38"
      "1e](as "
...
SCENARIO(
  ...
  GIVEN(
      "SFMC3019 sensor is fully initialised by calling setup method until it returns ok and action "
      "returns wait_measurement with the mock read buffer consisting of [15 35 a8 15 35"
      "a8](as read_sample response) followed by junk bytes [01 02], the mock time is set to"
      "100us, global device's mock write buffer is set to [01] byte and read buffer is set to [01 "
      "02] bytes"
      "and mock_device I2C's write buffer contains command byte [E1"
      "02](request_product_id), [36 "
      "61](read_conversion), [36 6A](set_averaging) and [36 03](O2 gas)") {
    PF::HAL::Mock::I2CDevice mock_device;
    PF::HAL::Mock::I2CDevice global_device;
    ...
    // Flow does not get updated
    WHEN(
        "The output method is called twice with mock time set to 40000us for both calls and output "
        "parameter flow is initialized to 20.5") {
      auto output_status = sensor.output(flow);
      THEN("The first output method call returns ok state") {
        REQUIRE(output_status == PF::InitializableState::ok);
...
//   GIVEN("SFMC3019 sensor is partially initialised by calling setup method once where it returns "
...
//      REQUIRE(input2[0] == 0x36);
//      REQUIRE(input2[1] == 0x03);
//      WHEN("The output method is called once, mock time is set to 250us and output parameter flow
...
//       [d5 64 4d](as " "read_sample response) and with the mock time set to 500us, global device's
//       mock write " "buffer is set to [01] byte and read buffer is set to [01 02] bytes" "and
//       mock_device I2C's write buffer contains command byte [E1 02](request_product_id), [36
//       61](read_conversion), [36 6A](set_averaging) and [36 03](O2 gas)") {
