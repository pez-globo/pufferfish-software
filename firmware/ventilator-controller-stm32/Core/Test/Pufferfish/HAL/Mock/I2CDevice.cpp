...
SCENARIO("I2C mock device works correctly for read method") {
  GIVEN("Freshly constructed mock device") {
    ...
    WHEN(
      ...
      THEN("The count value returns 54") { REQUIRE(count == 50); }
...
SCENARIO("Mock I2CDevice works correctly for read method when called three times") {
  GIVEN("Freshly constructed mock device") {
    ...
    WHEN(
        ...
        "bytes and [ok][ok][ok] I2CDeviceStatus") {
      PF::Util::Containers::ByteVector<buffer_size> input_read;
      PF::Util::Containers::ByteVector<buffer_size> input_read1;
      PF::Util::Containers::ByteVector<buffer_size> input_read2;
      PF::Util::Containers::ByteVector<buffer_size> input_read3;
...
SCENARIO("Mock I2CDevice works correctly for read method when called seven times") {
  GIVEN("Freshly constructed mock device") {
    ...
    size_t count = buffer_size;
    WHEN(
        "The add_read method is called eight times with input parametes,[27 b8] [64 f3] [2f 1c] "
        "[39 25] [f3 9e] [a9 78] [9c 7f][9c 7f] and [ok][ok][ok][ok][ok][ok][ok][ok] "
        "I2CDeviceStatus") {
      PF::Util::Containers::ByteVector<buffer_size> input_read;
      PF::Util::Containers::ByteVector<buffer_size> input_read1;
      PF::Util::Containers::ByteVector<buffer_size> input_read2;
      ...
      auto read_buffer7 = PF::Util::Containers::make_array<uint8_t>(0x9c, 0x7f);
      mock_device.add_read(read_buffer6.data(), read_buffer6.size(), add_status);
      mock_device.add_read(read_buffer7.data(), read_buffer7.size(), add_status);
      THEN("The count value returns 50") { REQUIRE(count == 50); }
...
SCENARIO(
  ...
  GIVEN("Freshly constructed mock device") {
    ...
    WHEN(
        ...
        "eb e0 73 a3 09 9a 4c 13 e3 ce 3f 9b 99 07 be be 5b 1d 7a 4a 8c ], followed by 25 data "
        "bytes [eb 65 a1 "
      ...
      THEN("The read method returns OK status") { REQUIRE(read_status == PF::I2CDeviceStatus::ok); }
      THEN(
          "The read method returns only first 50 data bytes [63 89 9a a9 27 54 0e 5e 90 c2 15 21 "
          "45 e1 "
...
SCENARIO("Mock device works correctly when write method is called four times") {
  GIVEN("Freshly constructed mock-device") {
    ...
    WHEN(
        "The write method is called five times with buffer bytes [db fe] [79 4b] [cd 93] [22 19] "
        "and I2CDevice status as [Ok] [write_error] [ok] [ok] ") {
      auto write_status = PF::I2CDeviceStatus::ok;
      mock_device.add_write_status(write_status);
      auto write_buffer = PF::Util::Containers::make_array<uint8_t>(0xdb, 0xfe);
      mock_device.write(write_buffer.data(), write_buffer.size());
...
SCENARIO(
  ...
  GIVEN("Freshly constructed mock device") {
    ...
    WHEN(
        ...
        "e0 73 a3 09 9a 4c 13 e3 ce 3f 9b 99 07 be ] followed by 25 data bytes [eb 65 a1 62 93 13 "
        "33 23 ff 97 ba "
      ...
      THEN(
          "The get_write buffer contains only first 50 data bytes [63 89 9a a9 27 54 0e 5e 90 c2 "
          "15 21 45 "
      ...
      THEN(
          "The get_write buffer contains expected 25 data bytes [eb 65 a1 62 93 13 33 23 ff 97 ba "
