...
//       THEN("The read method returns OK status") { REQUIRE(read_status ==
//       PF::I2CDeviceStatus::ok); } THEN("After the read method call, output buffer contains [a0 e8
//       d4 fd 97] bytes") {
...
//         " "7c] [d5 " "55] [2e 87] bytes and
//         [ok][read_error][invalid_arguments][crc_check_failed][no_new_data] " "I2CDeviceStatus and
//         count [2], then read method is called six times with " "input parameter count [2]") {
//       PF::HAL::Mock::I2CDevice mock_device;
...
//     }
//     WHEN(
//         "The add_read method is called four times with input parametes,[27 b8] [64 f3] [e1 7c]
//         [d5 " "55] bytes and [ok][read_error][invalid_arguments][ok]" "I2CDeviceStatus and count
//         [2], then read method is called five times " "with input parameter count [2]") {
//       PF::HAL::Mock::I2CDevice mock_device;
...
//     // WHEN(
//     //     "The add_read method is called three times with input parametes,[27 b8] [64 f3] [e1
//     7c] "
...
//     constexpr size_t buffer_size = 20UL;
//     WHEN(
//         "The add_read method is called with input parameters,[27 b8]bytes and [ok]
//         I2CDeviceStatus " "and count 2, and" "then read method is called with input parameter 2;
//         and then add_read and read " "methods are called in alternating order for 6 more " "times
//         where add_read method's input parameters are [64 f3 b3] "
...
// SCENARIO(
//     "I2C mock device works correctly for  add_read method when input parameter has differnt
//     buffer " "size, then read method is called repeatedly") {
//   GIVEN("Freshly constructed mock device") {
//     PF::HAL::Mock::I2CDevice mock_device;
...
//         " "and count [25] ,then read method is called thrice with " "input parameter count [50 25
//         25] respectively") {
//       auto status = PF::I2CDeviceStatus::ok;
//       size_t count = 50;
...
//       auto write_buffer = PF::Util::Containers::make_array<uint8_t>(0xd5, 0x1a, 0x7a, 0xbc,
//       0x24); auto status = mock_device.write(write_buffer.data(), write_buffer.size());
...
//         " "add_write_status method with input parameter" "not_supported I2CDeviceStatus is called
//         and then get_write method is called twice") {
...
//     WHEN(
//         "The write method is called with input parameters [d5 1a 7a bc 24]bytes and count [5],
//         and " "add_write_status method is called with input parameter" "no_new_data
//         I2CDeviceStatus, then get_write method is called twice") {
...
//         " "9d 1c] and count [4]" "and add_write_status's input parameters are [write_error] [OK]
//         I2CDeviceStatus") {
//       size_t count = 2;
//       auto write_status = PF::I2CDeviceStatus::ok;
//       mock_device.add_write_status(write_status);
...
//     " "input parameter " " of differnt buffer sizes ") {
//   GIVEN("Freshly constructed mock device") {
//     PF::HAL::Mock::I2CDevice mock_device;
//     constexpr size_t buffer_size = 50UL;
//     WHEN(
...
//         " "count [50] " "then add_write_status is called with input parameter [OK]
//         I2CDeviceStatus, then get_write " "method is called three times") {
//       auto status = PF::I2CDeviceStatus::ok;
...
//           "[63 ..ce 3f]") {
//         auto buffer = PF::Util::Containers::convert_byte_vector_to_hex_string(input_write,
//         count); const auto* expected =
//             R"(\x63\x89\x9A\xA9\x27\x54\x0E\x5E\x90\xC2\x15\x21\x45\xE1\xE1\xBB\x99\x55\x73\xA6\xA4\x04\x27\xAE\x2D\x1A\xF0\x43\xD6\x76\xD8\xB6\xCE\x30\xFC\x89\xE6\xF2\xE9\xEB\xE0\x73\xA3\x09\x9A\x4C\x13\xE3\xCE\x3F)";
//         REQUIRE(buffer == expected);  // 254 null bytes instead
//       }
//       count = 25;
